...
function yieldTreeSelect(items, textTpl, valueField, left) {
	...
	for (var a = 0; a < items.length; a++) {
		...
		for (var b in items[a]) text = text.replace(new RegExp('\{#' + b + '\}', 'g'), items[a][b]);
...
function cms2Filter(schema, query_all) {
	...
	for (var a = 0; a < schema.length; a++) {
		...
		var sb = '<div style="line-height:36px;float:left;margin-right:30px;">{0}： '.format(schema[a].name);
		for (var b = 0; b < schema[a].value.length; b++)
			sb += '<a id="a_{0}_id_{1}" name="a_{0}_id" field="{0}" value="{2}" href="#" style="padding:3px 6px 3px 6px;background-color:#eee;">{3}</a>&nbsp;'.format(schema[a].field, b, schema[a].value[b], schema[a].text[b]);
		sb += '</div>';
