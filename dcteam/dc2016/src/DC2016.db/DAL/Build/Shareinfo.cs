...
namespace DC2016.DAL {
	...
	public partial class Shareinfo : IDAL {
		#region transact-sql define
		public string Table { get { return TSQL.Table; } }
		public string Field { get { return TSQL.Field; } }
		public string Sort { get { return TSQL.Sort; } }
		internal class TSQL {
			internal static readonly string Table = "`shareinfo`";
			...
			public static readonly string Delete = "DELETE FROM `shareinfo` WHERE ";
			public static readonly string Insert = "INSERT INTO `shareinfo`(`extends`, `gate`, `guid`, `ip`, `number`, `server`, `shareid`, `state`, `time`, `used_number`, `used_userid`, `userid`) VALUES(?extends, ?gate, ?guid, ?ip, ?number, ?server, ?shareid, ?state, ?time, ?used_number, ?used_userid, ?userid); SELECT LAST_INSERT_ID();";
		}
		#endregion
		...
		public ShareinfoInfo GetItem(IDataReader dr) {
			int index = -1;
			return GetItem(dr, ref index) as ShareinfoInfo;
		}
		public object GetItem(IDataReader dr, ref int index) {
			return new ShareinfoInfo {
		...
		public SelectBuild<ShareinfoInfo> Select {
			get { return SelectBuild<ShareinfoInfo>.From(this, SqlHelper.Instance); }
		}
		#endregion
		...
		public partial class SqlUpdateBuild {
			...
			public override string ToString() {
				...
				if (string.IsNullOrEmpty(_where)) throw new Exception("防止 DC2016.DAL.Shareinfo.SqlUpdateBuild 误修改，请必须设置 where 条件。");
			...
			public SqlUpdateBuild Set(string field, string value, params MySqlParameter[] parms) {
				if (value.IndexOf('\'') != -1) throw new Exception("DC2016.DAL.Shareinfo.SqlUpdateBuild 可能存在注入漏洞，不允许传递 ' 给参数 value，若使用正常字符串，请使用参数化传递。");
			...
			public SqlUpdateBuild SetExtends(string value) {
				...
				return this.Set("`extends`", string.Concat("?extends_", _parameters.Count), 
					GetParameter(string.Concat("?extends_", _parameters.Count), MySqlDbType.VarChar, 50, value));
			}
		...
		public ShareinfoInfo GetItem(int? Id) {
